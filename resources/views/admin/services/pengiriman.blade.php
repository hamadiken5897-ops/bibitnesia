...
<div class="page-content">
    <div class="card">
        <div class="card-header pb-0 d-flex justify-content-between align-items-center">
            <h5 class="card-title mb-0">Daftar Seluruh Pengiriman</h5>
            <form action="{{ route('admin.pengiriman.index') }}" method="GET" class="d-flex gap-2">
                ...
                <input type="text" name="search" class="form-control form-control-sm" placeholder="Cari No. Resi / Pembeli / Penjual..." value="{{ request('search') }}">
                    ...
                    <tbody>
                        @forelse($pengiriman as $index => $item)
                            <tr>
                                ...
                                <td>{{ $item->pesanan->detailPesanan->first()?->produk->penjual->user->nama ?? 'N/A' }}</td>
                                ...
                                <td>
                                    ...
                                    @else
                                        <span class="badge bg-danger">{{ ucfirst($item->status_pengiriman) }}</span>
                                    @endif
                                </td>
                                <td>
                                ...
                                <td>
                                    <a href="{{ route('admin.pengiriman.show', $item->id_pengiriman) }}" class="btn btn-sm btn-outline-info">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center text-muted py-4">Belum ada data pengiriman</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
